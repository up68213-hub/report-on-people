...
import { ComboBox, NonNegativeIntegerInput, ReplaceOnFocusInput } from './UiControls.jsx';
...
export default function ManualEntryModal({ open, readOnly = false, userId, onClose, objects, initialObjectId, initialReportDate, notify, onSaved, onDashboard, onChangeContext }) {
  ...
  const setRating = (item, key, value) => setWeeklyQuality((current) => {
    const id = identity(item); const next = { ...current, [id]: { ...item, ...current[id], [key]: current[id]?.[key] === value ? '' : value } }; touchDraft(rows, next); return next;
  });
  ...
  return <>
    <div className="v4-entry-overlay" role="dialog" aria-modal="true" aria-label="Внести данные">
      <section className={`v4-sheet ${effectiveReadOnly ? 'is-readonly' : ''}`}>
        ...
        <div className="v4-tools"><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Поиск по виду работ или подрядчику" /><div className="v4-segments">{[['all', 'Все'], ['empty', 'Незаполненные'], ['dev', 'С отклонением']].map(([key, label]) => <button className={filter === key ? 'on' : ''} onClick={() => setFilter(key)} key={key}>{label}</button>)}</div></div>
    ...
    {qualityOpen && <QualityMatrix items={weekItems} quality={weeklyQuality} criteria={catalogs.qualityCriteria} onSet={setRating} onClose={() => setQualityOpen(false)} ratedCount={ratedCount} readOnly={effectiveReadOnly} />}
...
function QualityMatrix({ items, quality, criteria, onSet, onClose, ratedCount, readOnly }) {
  return <div className="v4-quality-overlay" role="dialog" aria-modal="true"><section className="v4-quality"><header><div><h2>Оценка <span>подрядчиков</span> за неделю</h2><p>{readOnly ? 'Оценка доступна только для просмотра' : 'Выберите формулировку в каждом блоке, балл считается автоматически'}</p></div></header><div className="v4-quality-scroll"><table><thead><tr><th>Категории и формулировка</th>{items.map((item) => <th key={identity(item)}>{item.contractor}<small>{item.workType}</small></th>)}</tr></thead><tbody>{criteria.map((criterion) => <QualityCriterion key={criterion.key} criterion={criterion} items={items} quality={quality} onSet={onSet} readOnly={readOnly} />)}</tbody></table></div><footer><span className={`v4-status ${ratedCount === items.length ? 'ok' : 'warn'}`}>{ratedCount === items.length ? `Отмечены все ${items.length} подрядчиков` : `Отмечено ${ratedCount} из ${items.length}`}</span><div><button className="flat" onClick={onClose}>Закрыть</button>{!readOnly && <button disabled={ratedCount !== items.length} onClick={onClose}>Сохранить оценки</button>}</div></footer></section></div>;
}
