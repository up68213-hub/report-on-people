...
const categoryNames = { work_type: 'Виды работ', cause: 'Причины', decision: 'Решения', contractor: 'Подрядчики', resource_measure: 'Меры департамента ресурсов' };
...
export default function AdminPage({ session, notify, onObjectsChanged, onRecordsChanged }) {
  ...
  const request = async (path, options, success, refreshObjects = false) => {
    try { await api(path, options); await load(); if (refreshObjects) await onObjectsChanged?.(); if (success) notify(success); return true; }
